{{-- 📌 Page entièrement faite à l'IA, pour avoir une structure cohérent avec les vues des postes. j'ai donc donner le dossier views/posts/* afin que l'IA puisse me refaire les fichier.blade.php selon le reste de l'application et au couleur de mon application --}}
        ...
        <article class="rounded-lg border border-[#E2B7A0] bg-white p-6 shadow-md">
            <header class="mb-6">
                ...
                <p class="mt-3 text-[#7C4A2D]">
                    Partage une routine adaptée aux cheveux afros, bouclés ou crépus.
                </p>
            </header>

            <form method="POST" action="{{ url('/routines') }}" class="space-y-5">
                @csrf
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4"
                        class="w-full rounded-md border border-[#E2B7A0] bg-[#FBF2E4] p-2 text-[#3F2A20] focus:border-[#7C4A2D] focus:ring-[#7C4A2D]">{{ old('description') }}</textarea>
                ...
                <div>
                    ...
                    <textarea name="steps" id="steps" rows="5"
                        class="w-full rounded-md border border-[#E2B7A0] bg-[#FBF2E4] p-2 text-[#3F2A20] focus:border-[#7C4A2D] focus:ring-[#7C4A2D]">{{ old('steps') }}</textarea>
                        ...
                        @foreach (['2A', '2B', '2C', '3A', '3B', '3C', '4A', '4B', '4C'] as $hairType)
                            <label class="flex items-center gap-2 text-[#3F2A20]">
                                <input type="checkbox" name="hair_types[]" value="{{ $hairType }}"
                                    ...
                                    {{ in_array($hairType, old('hair_types', [])) ? 'checked' : '' }}>
                                <span>{{ $hairType }}</span>
                            </label>
                        @endforeach
                    </div>
