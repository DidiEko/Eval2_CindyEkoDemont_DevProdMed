        ...
        <form method="POST" action="{{ url('/tokens') }}">
            ...
            <div class="mb-6">
                <fieldset>
                   ...
                   {{-- 📌 L’IA a été utilisée pour la génération d’un bloc répétitif. --}}

                    <div class="flex items-center mb-2">
    <input type="checkbox" id="scope-routines:create" name="scopes[]" value="routines:create"
        {{ in_array('routines:create', old('scopes', [])) ? 'checked' : '' }} class="mr-2">
    <label for="scope-routines:create" class="text-sm text-gray-700 dark:text-gray-300">
        Créer une routine
    </label>
</div>

<div class="flex items-center mb-2">
    <input type="checkbox" id="scope-routines:read" name="scopes[]" value="routines:read"
        {{ in_array('routines:read', old('scopes', [])) ? 'checked' : '' }} class="mr-2">
    <label for="scope-routines:read" class="text-sm text-gray-700 dark:text-gray-300">
        Lire les routines
    </label>
</div>

<div class="flex items-center mb-2">
    <input type="checkbox" id="scope-routines:update" name="scopes[]" value="routines:update"
        {{ in_array('routines:update', old('scopes', [])) ? 'checked' : '' }} class="mr-2">
    <label for="scope-routines:update" class="text-sm text-gray-700 dark:text-gray-300">
        Modifier les routines
    </label>
</div>

<div class="flex items-center mb-2">
    <input type="checkbox" id="scope-routines:delete" name="scopes[]" value="routines:delete"
        {{ in_array('routines:delete', old('scopes', [])) ? 'checked' : '' }} class="mr-2">
    <label for="scope-routines:delete" class="text-sm text-gray-700 dark:text-gray-300">
        Suprimer les routines
    </label>
</div>
                    @error('scopes')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
